  ...
  vis.bP = function(){
	  ...
	  function bP(_){
        ...
        _.each(function() {
         ...
         if(typeof fill !=="undefined") s.style("fill", d=>fill(d));
         ...
         if(typeof fill !=="undefined") e.style("fill", d=>fill(d));
	  ...
	  bP.keyPrimary = function(_){ 
		if(!arguments.length) return typeof keyPrimary !== "undefined" ? keyPrimary : (d=>d[0]) ;
		keyPrimary = _;
		return bP;		
	  }
	  bP.keySecondary = function(_){ 
		if(!arguments.length) return typeof keySecondary !== "undefined" ? keySecondary : (d=>d[1]);
		keySecondary = _;
		return bP;		
	  }	  
	  bP.value = function(_){ 
		if(!arguments.length) return typeof value !== "undefined" ? value : (d=>d[2]);
		value = _;
		return bP;		
	  }	  
	  bP.width = function(_){
	  ...
	  bP.bars = function(mb){
		...
		function calculateSubBars(part){
			function v(d){ return isSelKey(d, part) ? bP.value()(d): 0;};
			var map = d3.map(mainBars[part], d=>d.key);
		...
		function calculateEdges(){	
			var map=d3.map(subBars.secondary,d=>d.i);
		...
		function bpmap(a/*array*/, p/*pad*/, m/*min*/, s/*start*/, e/*end*/){
			...
			var ln =0, lp=0, t=d3.sum(a,_=>_.value); // left over count and percent.
			if(t<=0) return a;
